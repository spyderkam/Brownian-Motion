...
################  Bead Class  ################
class Bead:
  """A 2D bead w/initial position in Cartesian coordinates."""
  ...
  def force_calculate(self, j, jj=None, k=0, Ls=None, kBT=1, lk=1):     # avoid calling outside of 'advance' method
    if xs != None:
      if j == 0:
...
class Simulation:
  ...
  def advance(self, Δt, b=1, κ=0):
    ...
    for i in range(N-1):
      #ave_bead_dist = np.sqrt(end_to_end[i][0]**2 + end_to_end[i][1]**2)/self.nbeads
      for n, bead in enumerate(self.beads):
        bead.x = bead.x + (bead.force_calculate(k=κ, j=n, jj=i, Ls=3, lk=.1)[0] / b)*Δt # Ls != self.nbeads*l0
        bead.y = bead.y + (bead.force_calculate(k=κ, j=n, jj=i, Ls=3, lk=.1)[1] / b)*Δt
